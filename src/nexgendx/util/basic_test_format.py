#!/usr/bin/env python3
""" | A widely applicable module defining a myriad of 
    | useful functions for bioinformatics analysis    
    | scripting.                                      
"""
...
def deprecated(func):
    ...
    def new_func(*args, **kwargs):
        if not new_func.__called:
            warnings.warn('Call to deprecated function "{0}".'.format(
                func.__name__), stacklevel=2)
...
def get_tert_by_sample(Oracle_con):
    ...
    df_T1 = pd.read_sql_query(
        "select * from CFDNA_SAMPLE_RESULT_T1", Oracle_con)
    df_T2 = pd.read_sql_query(
        "select * from CFDNA_SAMPLE_RESULT_T2", Oracle_con)
...
def print_progress(message, init, n_smpls):
    ...
    print(
        "\n{:^150s}\n\n{:^150s}\n{:^150s}\n\n{:^150s}\n".format(
            banner, message + str(round(time.time() - init, 5)) + " s",
            "N(smpls) = " + str(n_smpls),
            banner),
        file=sys.stderr)
...
spectrum1 = [
    (0.725467917633003, 0.10980392156862745, 0.2196078431372549),
    '#1e488f', '#be03fd',
